<script lang="ts" setup>
    ...
    try {
        ...
        const data = {
            wood_name: formData.value.wood_name
        };

        console.log('Data yang dikirim:', data);

        const response = await axios.post('https://api.udpadijaya.com/api/woods', data, {
            headers: {
...
<template>
    <VueFinalModal  
        ...
        <form class="flex flex-col justify-center gap-2 lg:gap-5 max-w-4xl" @submit.prevent="submitForm" action="" method="post">
            <h1 class="lg:py-5 w-auto lg:text-3xl lg:px-10 font-dm text-color2 font-bold text-center">Tambah Kayu</h1>

            <div class="w-full px-5 flex flex-col gap-2">
                <label for="">Kayu</label>
                <input v-model="formData.wood_name" class="text-sm w-full h-11 font-dm border px-3 text-color6 placeholder:text-color4 rounded-lg focus:ring-color3 focus:border-color2" placeholder="Masukkan Jenis Kayu" type="text" />
            </div>
