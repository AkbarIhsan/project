...
<template>
    <div class="w-full h-auto justify-center items-center flex fixed z-50">
        <div class="absolute h-full flex items-center lg:left-20">
            <h1 class="lg:text-2xl font-extrabold text-color6">UD. PADI JAYA</h1>
        </div>
        ...
        <form v-if="isSearchOpen" class="absolute w-[80%] top-20 transition-all">
            ...
            <div class="relative flex items-center">
                ...
                <input type="search" id="default-search" class="shadow-full shadow-black/20 w-full py-3 ps-10 text-sm text-gray-900 border border-color6 rounded-full bg-gray-50 focus:ring-color6 focus:border-color6 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-color6 dark:focus:border-color6" placeholder="Kayu Jati, Kayu Tangan,.." required />
        ...
        <nav v-if="isMenuOpen" class="absolute w-[95%] top-20 bg-white shadow-lg md:hidden rounded-lg z-50">
            <ul class="flex flex-col items-start p-4 space-y-2 text-sm font-bold text-color4">
                <li>
                    <NuxtLink @click="toggleMenu" :class="{'text-color8' : isActive('/')}" class="hover:text-color6" to="/">HOME</NuxtLink>
                </li>
                <li>
                    <NuxtLink @click="toggleMenu" :class="{'text-color8' : isActive('/about')}" class="hover:text-color6" to="/about">ABOUT US</NuxtLink>
                </li>
                <li>
                    <NuxtLink @click="toggleMenu" :class="{'text-color8' : isActive('/product')}" class="hover:text-color6" to="/product">PRODUCT</NuxtLink>
                </li>
            </ul>
        </nav>
        ...
        <nav :class="['transition-all duration-300 flex items-center w-full', {'border-none h-16 sm:h-16 md:h-16 lg:h-20 bg-none': !isScrolled, 'bg-white border shadow-full shadow-black/5 h-16 sm:h-16 md:h-16 lg:h-16 rounded-b-full':isScrolled}]">
            <ul class="hidden lg:flex w-full h-[50%] justify-center items-center text-xs lg:text-sm font-bold tracking-widest gap-3 text-color4">
                ...
                <li class="h-full md:px-5 lg:px-7 flex items-center">
                    <NuxtLink :class="{'hover:border-none text-color8' : isActive('/product')}" class="hover:border-b hover:border-b-color6" to="/product">PRODUCT</NuxtLink>
                </li>
            </ul>
        </nav>
    </div>
